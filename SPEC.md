...
C8|Node ≥ 20 LTS; repository `npm test` = `node:test`, scope = Agent Skills contract (§V1-4) + real CLI discovery (§V5) only; license/release/hygiene = manual review, ⊥ test-backed; ⊥ assert doc prose \| skill-body wording. Separate `benchmark/` simulations assess generated code and observed workflow behavior; ⊥ add fixture tests to repository test discovery
...
I13|skill|`/review-vibe` → standalone current-codebase review + direct evidenced fixes; ⊥ invoked by another skill; explicit, self-contained coverage of security, test value, complexity/reuse, service/provider contracts, file organization, and spec coherence. Simple, bounded fixes in a small codebase MAY omit `REVIEW.md` when examination, fixes, and verification finish in one session with results in the report. Broader, expanded, blocked, or resumable reviews retain coverage, findings, checks, and next action in root `REVIEW.md`; preserve unrelated ledger content and continue sections/rounds toward full scoped coverage without repeated user instructions. ⊥ require new plan; durable spec writes via `encode-docs`
I14|folder|`benchmark/` → small reproducible coding prompts, fixture definitions, evaluation criteria, and evidenced agent-run comparisons; workflow = `prep` → `review-plan` → `cook` or `cater` → `review-code` → gated `garnish`; main agent independently checks generated code, verification, and workflow evidence. Runs isolated from product skills and repository tests; `review-vibe` checks remain standalone
...
V19|`PLAN.md` (phase plan; owns §T; final phase verifies) + `HANDOFF.md` (session baton) = short-lived cycle files; ∀ writes through `encode-docs`. Dedicated first research phase MAY be omitted when `prep` or `review-plan` completed scope-relevant research and recorded current evidence; research itself ⊥ omitted. Preserve retained task ids, evidence, and valid pointers
...
V21|`prep` → durable facts → `SPEC.md` via `encode-docs` (high bar, ⊥ default); §T tasks authored in `PLAN.md`; ∀ phase ≥1 task, ids `T<n>` monotonic within phase, ∀ task cites relevant §V. Research by `prep` or `review-plan` precedes coding execution; record scope, sources/inspection, findings, unresolved items, and evidence currency in plan. Completed research needs no repeated confirmation phase; final verification remains last. Embed 1 `review-plan` pass → matching `handoff`
...
V24|`review-plan` → resolve research with repository evidence and current primary sources when external behavior matters (date external checks; ⊥ unsupported model memory); explicit GO/NO-GO. `cook`/`cater` validate prior research before execution; missing/stale evidence → main-agent `review-plan` before coding; unresolved consequential unknowns block dependent work. A phase title or unchecked research claim ⊥ proof

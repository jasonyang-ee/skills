...
- Make the smallest coherent change that satisfies the task. Search for existing helpers and inspect callers before adding abstractions; follow neighboring naming and error-handling conventions.
...
3. Load `encode-docs`. Use an explicit phase argument if supplied. Otherwise validate the handoff pointer against task status and dependencies; fall back to the first unfinished eligible phase in plan order. A stale pointer never overrides the plan.
4. Before any execution-state transition or phase work, validate the plan's prior-research record for the selected scope: questions/findings/decisions, local paths and relevant revisions/dirty inputs, applicable external URLs/check dates, remaining unknowns, and gate. Check changes to relevant requirements, files, callers, dependencies, and source currency against affected assumptions. Refresh only evidence whose findings/decisions no longer support selected work; unrelated changes and verified planned edits preserving that support do not stale research. Missing or stale evidence → load and run `review-plan` in the main agent first, then reselect work from the corrected plan. Reuse valid prep/review-plan evidence without a mandatory research or confirmation phase.
5. Run only tasks whose dependencies and gates are satisfied, including unresolved blockers from plan review. Consequential unknowns block dependent coding; bounded research or explicitly independent work may proceed when its own evidence and gates permit. Before the first eligible phase, request `new` → `work-in-progress`; this skill owns the transition, and the encoder performs the write. Record skipped blocked work; never bypass a dependency to reach final verification. Repeat the evidence check when selecting each phase or when relevant inputs change.
...
3. Implement the task and run required checks. For behavioral checks, inspect the actual cases/assertions and confirm they ran and exercise the changed behavior; exit zero alone is not proof. Classify failures as implementation defect, requirement conflict, unspecified behavior, pre-existing failure, or environment limitation. Investigate before retrying. Fix in-scope defects; record other failures and their effect on completion.
4. Read the full owned diff and relevant surrounding context. Check requirements, logic, reuse, interfaces, error handling, and security boundaries. Remove dead/debug code, unrelated edits, secret material, and unnecessary complexity; verify any new untrusted-input path has the validation its boundary requires. Repeat affected checks after corrections.

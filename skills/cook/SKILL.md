...
Use `cook` when any of these are true:
...
- A cold follow-up session should be able to start with `/workonplan` and no
  extra explanation.
...
Every `cook` run must produce all of these:
...
`spec` remains the sole mutator of `SPEC.md`. `cook` prepares the material and

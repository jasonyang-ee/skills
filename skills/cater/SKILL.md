...
Require an executable plan. `new` without phases or no plan → `/prep`; `done` → `/garnish`; `new` with phases or `work-in-progress` → eligible for execution after evidence checks. Reconcile inconsistent state before starting. Load `encode-docs`.
...
Before an execution-state transition, direct work, or dispatch, validate the plan's prior-research record for the selected scope: questions/findings/decisions, local paths and relevant revisions/dirty inputs, applicable external URLs/check dates, remaining unknowns, and gate. Compare relevant requirements, files, callers, dependencies, and source currency against affected assumptions. Refresh only evidence whose findings/decisions no longer support selected work; unrelated changes and verified planned edits preserving that support do not stale research. Missing or stale evidence → load and run `review-plan` in the main agent first, then reselect work from the corrected plan. Reuse valid prep/review-plan evidence without a mandatory research or confirmation phase. Consequential unknowns block dependent coding; bounded research or explicitly independent work may proceed when its own evidence and gates permit. Only then request `new` → `work-in-progress` before eligible phase work. Repeat this check for each selected phase or when relevant inputs change.
...
4. Collect results and mark them unreviewed in the main baton. Read the entire scoped diff and relevant context; check logic and surrounding-code coherence, then verify every assigned requirement and applicable implementation check carried by the loaded `encode-agent` contract, including scope, conventions/reuse, diff hygiene, and input boundaries. For behavioral checks, inspect the actual cases/assertions and confirm they ran and exercise the changed behavior; a worker report or exit zero alone is not proof. Run required acceptance checks, repeating broader checks only when evidence warrants it. Research or verification tasks may validly finish without an implementation diff.

...
- fix cater to increase execution of ALL remaining phases
...
- Keep `.claude-plugin/plugin.json` synchronized with `package.json` when `release.sh` cuts a release, so Claude Code users receive plugin updates.
...
[Unreleased]: personal:jasonyang-ee/skills/compare/v0.6.1...HEAD
